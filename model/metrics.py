...
def prec_at_ks(true_r, pred_r, norm, ks, rm, print_ids=[]):
    ...
    ps = np.zeros((m, n, len(ks)))
    for i in range(m):
        for j in range(n):
            for k_idx, k in enumerate(ks):
                assert (type(k) is int and k > 0 and k < l)
                true_ids = true_r.top_k_ids(i, j, k, norm, inclusive=True, rm=rm)
                pred_ids = pred_r.top_k_ids(i, j, k, norm, inclusive=True, rm=rm)
                ps[i][j][k_idx] = \
                    min(len(set(true_ids).intersection(set(pred_ids))), k) / k
            if i in print_ids:
                print('query1 {}\nquery2 {}\nks:    {}\nprecs: {}'.format(i, j, ks, ps[i][j]))
    return np.mean(ps, axis=(0,1))
...
def accuracy(true_r, pred_r, thresh_pos, thresh_neg,
    ...
    for i in range(m):
        for j in range(n):
            for k in range(l):
                true_label = true_label_mat[i][j][k]
                pred_label = pred_label_mat[i][j][k]
                if true_label == 1:
                    if pred_label == 1:
                        tps += 1
                    else:
