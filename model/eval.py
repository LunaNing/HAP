...
from utils_model import get_flags, load_data
...
class Eval(object):
    def __init__(self, train_graphs, test_graphs):
        test_gs1 = load_data(FLAGS.dataset, train=False).graphs#from test graph
        test_gs2 = load_data(FLAGS.dataset, train=True).graphs#from train graph
        ...
        print('true val result loaded')
        self.true_test_result = load_result(
            FLAGS.dataset, FLAGS.dist_algo
            , row_graphs=test_gs1, col_graphs=test_gs2)
        # self.true_test_result = load_result(
        #     FLAGS.dataset, FLAGS.dist_algo,
        #     dist_mat=self._get_true_dist_mat_for_test(data, dist_calculator),
        #     row_graphs=test_gs1,
        #     col_graphs=test_gs2)
        print('true test result loaded')
        self.norms = [FLAGS.dist_norm]

    def get_val_gs_as_tuple(self, data):
        return data.val_graphs, data.train_graphs

    def get_test_gs_as_tuple(self, data):
        return data.test_graphs, data.train_graphs + data.val_graphs
    ...
    def _get_true_dist_mat_for_test(self, data, dist_calculator):
        gs1, gs2 = self.get_test_gs_as_tuple(data)
        gs1 = self._to_nxgraph_list(gs1)
        gs2 = self._to_nxgraph_list(gs2)
        return get_gs_dist_mat(gs1, gs2, dist_calculator, 'test', 'train',
                               FLAGS.dataset, FLAGS.dist_metric,
                               FLAGS.dist_algo, norm=False)
    ...
    def get_true_sim(self, i, j, k, val_or_test, model):
        if val_or_test == 'val':
            r = self.true_val_result
        else:
            assert (val_or_test == 'test')
            r = self.true_test_result
        return model.get_true_sim(i, j, k, r)
