from utils_model import get_result_path, get_save_path, get_model_path, \
    get_file_base_id, load_data, load_pkl, save_pkl
from distance import normalized_ged3, normalized_ged
...
class Result(object):
    ...
    def dist_sim(self, qid, gid, pid, norm):
        """
    ...
    def top_k_ids(self, qid, pid, k, norm, inclusive, rm):
        """
        :param qid: query id (0-indexed).
        :param k: 
        :param norm: 
        ...
        m, n, l = sort_id_mat.shape
        if k < 0 or k >= l:
            raise RuntimeError('Invalid k {}'.format(k))
        if not inclusive:
            return sort_id_mat[qid][pid][:k]
        # Tie inclusive.
        dist_sim_mat = self.dist_sim_mat(norm)
        while k < l:
            cid = sort_id_mat[qid][pid][k - 1]
            nid = sort_id_mat[qid][pid][k]
            if abs(dist_sim_mat[qid][pid][cid] - dist_sim_mat[qid][pid][nid]) <= rm :
                k += 1
            else:
                break
        return sort_id_mat[qid][pid][:k]
...
class DistanceModelResult(Result):
    def __init__(self, dataset, model, dist_mat, row_graphs, col_graphs):
        ...
        self.model_ = model
        if dist_mat is not None:
            self.dist_mat_origin = dist_mat
            self.time_mat_ = None
        else:
            self.dist_mat_origin = self._load_result_mat(
                self.dist_metric(), self.model_, len(row_graphs), len(col_graphs))
            self.time_mat_ = self._load_result_mat(
                'time', self.model_, len(row_graphs), len(col_graphs))
        # self.dist_norm_mat_ = np.copy(self.dist_mat_)
        m, n = self.dist_mat_origin.shape
        assert (m == len(row_graphs))
        assert (n == len(col_graphs))
        sim_mat = np.zeros((m, n, n))
        self.dist_norm_mat_origin = np.zeros((m, n))
        self.dist_norm_mat_ = np.zeros((m, n, n))
        for i in range(m):
            for j in range(n):
                d_origin = self.dist_norm_mat_origin[i][j]
                self.dist_norm_mat_origin[i][j] = normalized_ged(d_origin, row_graphs[i], col_graphs[j])
                for k in range(n):
                    if k != j:
                        true_sim12 = self.dist_mat_origin[i][j]
                        true_sim13 = self.dist_mat_origin[i][k]
                        true_sim = true_sim12 - true_sim13
                        sim_mat[i][j][k] = true_sim
                        self.dist_norm_mat_[i][j][k] = normalized_ged3(
                            true_sim, row_graphs[i], col_graphs[j], col_graphs[k])
        self.dist_mat_ = sim_mat
        self.sort_id_mat_ = np.argsort(self.dist_mat_, kind='mergesort')
        self.dist_norm_sort_id_mat_ = np.argsort(self.dist_norm_mat_, kind='mergesort')
    ...
    def dist_mat_originn(self, norm):
        return self._select_dist_mat_origin(norm)

    def dist_sim_mat(self, norm):
        return self.dist_mat(norm)

    def dist_sim_mat_origin(self, norm):
        return self.dist_mat_originn(norm)

    def dist_or_sim(self):
        return 'dist'

    def dist_sim(self, qid, gid, pid, norm):
        return self.dist_metric(), self._select_dist_mat(norm)[qid][gid][pid]
    ...
    def _load_result_mat(self, metric, model, m, n):
        file_p = get_result_path() + '/{}/{}/{}_{}_mat_{}_{}.npy'.format(
    ...
    def _select_dist_mat_origin(self, norm):
        return self.dist_norm_mat_origin if norm else self.dist_mat_origin

    def _select_sort_id_mat(self, norm):
        return self.dist_norm_sort_id_mat_ if norm else self.sort_id_mat_


class TripleGEDModelResult(DistanceModelResult):
    def dist_metric(self):
        return 'ged'
...
def load_result(dataset, model, sim=None, sim_mat=None, dist_mat=None,
                row_graphs=None, col_graphs=None,
                time_mat=None, model_info=None):
    if 'beam' in model or model in ['astar', 'hungarian', 'vj', 'simgnn']:
        return TripleGEDModelResult(
            dataset, model, dist_mat, row_graphs, col_graphs)
    elif model == 'graph2vec':
        return Graph2VecResult(dataset, model, sim)
    elif model == 'regression' or model == 'classification':
